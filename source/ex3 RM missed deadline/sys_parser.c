...
char Op; int time_unit;char task_selected[5] ;  
}event;
...
{
	int i,task_position;
	doc = xmlReadFile( "exo2_1tuto.xml", NULL, 0 );
	xmlNode* root = NULL;
	...
	char tasks_map[nmbT][5];
	
	exo1 = Create_system(core_id,nmbT);

	for( i=0; i<nmbT; i++)
		{
		strcpy(tasks_map[i],tmp[i].Id );  // storing the Ids of the tasks in order to obtain the position 	
	...
	doc = xmlReadFile( "trace_exo2_1tuto.xml", NULL, 0 );
	root = NULL;
	if ( doc==NULL ) return 0;
	root = xmlDocGetRootElement( doc );
	get_trace(root);	
...
for (i=0;i<nmbEvents;i++)
	...
	for(j=0;j < exo1-> numberOfTasks;j++){ 
		...
		{
		if(Check_activation(exo1,j)) printf ("?!?!?!?!?no activation alert task %s ?!?!?!?!?	current_time:	%d\n",tasks_map[j],exo1 -> current_time_unit);
	        if(Check_deadline(exo1,j)) printf ("?!?!?!?!?deadline alert task %s !?!?!?!?!?!?!?!?	current_time:	%d\n",tasks_map[j],exo1 -> current_time_unit);
		Reinit_task ( exo1, j);			
		}
