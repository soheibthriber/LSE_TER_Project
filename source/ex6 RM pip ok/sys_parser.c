...
for ( current=root; current!=NULL; current=current->next )
		{
			...
			if ( strcmp((char*)current->name,"pip_resource")==0 )	 
				{
				nmb_Res++;pos_Res++;
...
int main()
{
	...
	doc = xmlReadFile( "exo4_2tuto.xml", NULL, 0 );
	xmlNode* root = NULL;
	...
	for( i=0; i<nmb_Res; i++)
		{
		//strcpy(resources_map[i],tmp_res_tab[i].res_Id);		
...
//*******************************************************************************************************
	...
	doc = xmlReadFile( "trace_exo4_2tuto.xml", NULL, 0 );
	root = NULL;
	if ( doc==NULL ) return 0;
	root = xmlDocGetRootElement( doc );
	get_trace(root);	
...
for (i=0;i<nmbEvents;i++)
	...
	switch(event_tab[i].Op) {
		...
		case 'C':
			allocate_resource(exo1,task_position,event_tab[i].res_selected);
		break;

		case 'L':
			release_resource(exo1,task_position,event_tab[i].res_selected);
		break;
	
		case 'W':
			wait_for_resource(exo1,task_position,event_tab[i].res_selected);
		break;
